      ...
      <div className="container p-6">
        <Row gutter={[16, 16]}>
          <CardInitial
            ...
            boxShadow={true}
            category={
              <span className="text-white w-fit bg-blue-50 rounded-3xl text-xs py-1 px-2">
                {" "}
                مقالات{" "}
              </span>
            }
          ...
          <CardWidthTags
            ...
            boxShadow={true}
            category={
              <span className="text-white w-fit bg-red-700 absolute top-4 left-0  rounded-tr-lg rounded-br-lg text-sm py-1 px-4">
                {" "}
                مقالات{" "}
              </span>
            }
...
const CardInitial: React.FC<CardInitialType> = ({
  cardImage,
  title,
  description,
  textColor,
    ...
    <>
      <Col xs={24} md={12} lg={8}>
        <Link to={"/"}>
          <div
            className={`${
              boxShadow && "shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px]"
            }  w-full rounded-lg borderCard`}
          >
            ...
            <div className="flex flex-col px-4 py-2">
              ...
              <p
                className={`${
                  textColor ? textColor : "text-zinc-950"
                } font-bold text-2xl line-clamp-1 leading-8 py-2`}
              >
                {" "}
                {title}
              </p>
            ...
            <div
              className={`${
                userName ? "flex  items-center" : "flex"
              } px-4 py-2`}
            >
              {profile && (
                <img
              ...
              <div className="flex flex-col">
                <span
                  className={`${
                    authorTextColor ? authorTextColor : textColor
                  } text-sm pb-2 `}
                >
                  {userName}
                </span>
...
const CardWidthTags: React.FC<CardInitialType> = ({
  cardImage,
  title,
  description,
  textColor,
          ...
          >
            ...
            <div className="flex flex-col px-4 py-2">
              <div className="flex">
                {tags &&
                  tags.map((tag, i) => (
                    <span key={i + "tag"} className="text-blue-50">
                      {" "}
                      #{tag}
                    </span>
                  ))}
...
export const ShoppCard: React.FC<ShoppCardProps> = ({
  orderImage,
  orderName,
  orderPrice,
  priceColor,
    ...
    <Col xs={24} md={12} lg={8} xl={6}>
      <div className="w-full shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] rounded-lg ">
        <div className="group w-full relative pt-[75%] overflow-hidden rounded-lg">
          <img
            className="w-full h-full  object-cover absolute group-hover:opacity-35 top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]"
            width={100}
            height={100}
            src={`../../../../public/file/${orderImage}`}
            alt={orderImage}
          />
          {category && (
            <span className="text-white bg-orang-100 absolute top-4 z-10  rounded-lg px-2">
              {" "}
              {category}
            </span>
          )}
          <div className="absolute opacity-0 group-hover:opacity-100   z-10 transition ease-in-out delay-150 top-[50%]  left-[50%] translate-x-[-50%] translate-y-[-100%] group-hover:translate-y-[-50%]">
            <BtnBasComponent
              btnEvent={() => viewButtonFn(productId)}
        ...
        <div className="flex flex-col px-4 py-2">
          <span className="text-lg leading-7 py-1 line-clamp-1 ">
            {orderName}
          </span>
          <div className="flex justify-between mb-2 mt-4">
            <span>
              {" "}
              قیمت :{" "}
              <span className={`${priceColor}`}>{orderPrice} هزارتومان </span>
            </span>
            {rate && (
              <p className=" flex">
                {" "}
                <IoIosStar size={18} color="#ffd70047" />{" "}
                <span className="px-2 text-gray-400">{rate.rate}</span>
              </p>
            )}
          </div>
        </div>
